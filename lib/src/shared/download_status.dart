...
class AssetsDownloader extends Downloader {
  ...
  @override
  Future<String?> downloadAssets(String assetUrl, String pathToSave) async => await FlutterDownloader.enqueue(url: assetUrl, savedDir: pathToSave);

  @override
  Future<DownloadStatus> getdownloadStatusById(String taskId) async {
    var tasks = await FlutterDownloader.loadTasks();
    if (tasks != null && tasks.isNotEmpty) {
      return tasks.where((task) => task.taskId == taskId).first.progress == 100 ? DownloadStatus.downlaoded : DownloadStatus.downlaoding;
    }
